...
class Pad {
  ...
  @override
  bool operator ==(Object other) => identical(this, other) || other is Pad &&
     other.id == id &&
     other.url == url &&
  ...
  @override
  String toString() => 'Pad[id=$id, url=$url, agencyId=$agencyId, name=$name, infoUrl=$infoUrl, wikiUrl=$wikiUrl, mapUrl=$mapUrl, latitude=$latitude, longitude=$longitude, location=$location, countryCode=$countryCode, mapImage=$mapImage, totalLaunchCount=$totalLaunchCount, orbitalLaunchAttemptCount=$orbitalLaunchAttemptCount]';
  ...
  /// Returns a new [Pad] instance and imports its values from
  /// [value] if it's a [Map], null otherwise.
  // ignore: prefer_constructors_over_static_methods
  static Pad? fromJson(dynamic value) {
    if (value is Map) {
      ...
      assert(() {
        requiredKeys.forEach((key) {
          assert(json.containsKey(key), 'Required key "Pad[$key]" is missing from JSON.');
          assert(json[key] != null, 'Required key "Pad[$key]" has a null value in JSON.');
        });
        return true;
      }());
  ...
  static List<Pad> listFromJson(dynamic json, {bool growable = false,}) {
    final result = <Pad>[];
    if (json is List && json.isNotEmpty) {
      for (final row in json) {
        final value = Pad.fromJson(row);
        if (value != null) {
          result.add(value);
        }
      }
  ...
  static Map<String, Pad> mapFromJson(dynamic json) {
    final map = <String, Pad>{};
    if (json is Map && json.isNotEmpty) {
      json = json.cast<String, dynamic>(); // ignore: parameter_assignments
      for (final entry in json.entries) {
        final value = Pad.fromJson(entry.value);
        if (value != null) {
          map[entry.key] = value;
        }
      }
  ...
  // maps a json object with a list of Pad-objects as value to a dart map
  static Map<String, List<Pad>> mapListFromJson(dynamic json, {bool growable = false,}) {
    final map = <String, List<Pad>>{};
    if (json is Map && json.isNotEmpty) {
      // ignore: parameter_assignments
      json = json.cast<String, dynamic>();
      for (final entry in json.entries) {
        map[entry.key] = Pad.listFromJson(entry.value, growable: growable,);
      }
    }
    return map;
  }
