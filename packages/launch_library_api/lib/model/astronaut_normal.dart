...
class AstronautNormal {
  /// Returns a new [AstronautNormal] instance.
  AstronautNormal({
    required this.id,
    required this.url,
    required this.name,
  ...
  @override
  bool operator ==(Object other) => identical(this, other) || other is AstronautNormal &&
     other.id == id &&
     other.url == url &&
     other.name == name &&
     other.status == status &&
  ...
  @override
  String toString() => 'AstronautNormal[id=$id, url=$url, name=$name, status=$status, type=$type, inSpace=$inSpace, timeInSpace=$timeInSpace, evaTime=$evaTime, age=$age, dateOfBirth=$dateOfBirth, dateOfDeath=$dateOfDeath, nationality=$nationality, bio=$bio, twitter=$twitter, instagram=$instagram, wiki=$wiki, agency=$agency, profileImage=$profileImage, profileImageThumbnail=$profileImageThumbnail, flightsCount=$flightsCount, landingsCount=$landingsCount, spacewalksCount=$spacewalksCount, lastFlight=$lastFlight, firstFlight=$firstFlight]';
  ...
  /// Returns a new [AstronautNormal] instance and imports its values from
  /// [value] if it's a [Map], null otherwise.
  // ignore: prefer_constructors_over_static_methods
  static AstronautNormal? fromJson(dynamic value) {
    if (value is Map) {
      ...
      assert(() {
        requiredKeys.forEach((key) {
          assert(json.containsKey(key), 'Required key "AstronautNormal[$key]" is missing from JSON.');
          assert(json[key] != null, 'Required key "AstronautNormal[$key]" has a null value in JSON.');
        });
        return true;
      }());

      return AstronautNormal(
  ...
  static List<AstronautNormal> listFromJson(dynamic json, {bool growable = false,}) {
    final result = <AstronautNormal>[];
    if (json is List && json.isNotEmpty) {
      for (final row in json) {
        final value = AstronautNormal.fromJson(row);
        if (value != null) {
          result.add(value);
        }
      }
  ...
  static Map<String, AstronautNormal> mapFromJson(dynamic json) {
    final map = <String, AstronautNormal>{};
    if (json is Map && json.isNotEmpty) {
      json = json.cast<String, dynamic>(); // ignore: parameter_assignments
      for (final entry in json.entries) {
        final value = AstronautNormal.fromJson(entry.value);
        if (value != null) {
          map[entry.key] = value;
        }
      }
  ...
  // maps a json object with a list of AstronautNormal-objects as value to a dart map
  static Map<String, List<AstronautNormal>> mapListFromJson(dynamic json, {bool growable = false,}) {
    final map = <String, List<AstronautNormal>>{};
    if (json is Map && json.isNotEmpty) {
      // ignore: parameter_assignments
      json = json.cast<String, dynamic>();
      for (final entry in json.entries) {
        map[entry.key] = AstronautNormal.listFromJson(entry.value, growable: growable,);
      }
    }
    return map;
  }
